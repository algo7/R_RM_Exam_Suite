...
menuListT1<-c(
  ...
  'Pick-Up Matrix (Average Booking Curve Given)',
  'Back'
)
...
abt_given <- function(){
  ...
  x<-fileImport(FALSE)
  # Convert it to data frame
  dfc<-data.frame(x,row.names = 1)
  # Set col. names
  colnames(dfc) <- dfc[1,]
  dfc <- dfc[-1,]
  # Pickup matrix
  PPM<-data.frame(round(dfc[,1]-dfc))
  # Write to csv
  openxlsx::write.xlsx(PPM,'./examples/forecasting/pmx.xlsx')
  # Print result
  cli_alert_success('Pickup Matrix: ')
  cat('\n')
  print(PPM)
  cat('\n')
  print('Find the Pickup Matrix at ./examples/forecasting/pmx.xlsx')
  cat('\n')
